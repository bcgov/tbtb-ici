...
RUN export ORACLE_HOME=/usr/lib/oracle/12.2/client64:$ORACLE_HOME
RUN export LD_LIBRARY_PATH=/usr/lib/oracle/12.2/client64/lib:$LD_LIBRARY_PATH

RUN pear download pecl/oci8-3.2.1 && cd /var/www/html && tar xvzf oci8-3.2.1.tgz && cd oci8-3.2.1 && phpize && ./configure --with-oci8=instantclient,/usr/lib/oracle/12.2/client64/lib/ && make
